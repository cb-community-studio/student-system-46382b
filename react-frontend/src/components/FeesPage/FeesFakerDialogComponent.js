...
import feesFakerFactory from "./feesFakerFactory";
...
const FeesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = feesFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("fees").create(elem));
...
export default connect(null, mapDispatch)(FeesFakerDialogComponent);
