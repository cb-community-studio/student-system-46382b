...
import programFakerFactory from "./programFakerFactory";
...
const ProgramFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = programFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("program").create(elem));
...
export default connect(null, mapDispatch)(ProgramFakerDialogComponent);
