...
import seederData from "./enrolment.seeder.json";
...
const EnrolmentSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("enrolment").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(EnrolmentSeederDialogComponent);
