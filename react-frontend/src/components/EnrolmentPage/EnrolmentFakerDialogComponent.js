...
import enrolmentFakerFactory from "./enrolmentFakerFactory";
...
const EnrolmentFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = enrolmentFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("enrolment").create(elem));
...
export default connect(null, mapDispatch)(EnrolmentFakerDialogComponent);
