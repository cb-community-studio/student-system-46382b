...
const ApplicationCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            applicationid: _entity.applicationid,
    ...
    return (
        <Dialog header="Create" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div role="application-create-dialog-component">
                <div>
                    <p className="m-0" >applicationid:</p>
                    <InputText className="w-full mb-3" value={_entity?.applicationid} onChange={(e) => setValByKey("applicationid", e.target.value)}  />
                </div>
                <div>
                ...
                <div>
                    <p className="m-0" >EligibilityStatus:</p>
...
export default connect(null, mapDispatch)(ApplicationCreateDialogComponent);
