...
import seederData from "./studentinfo.seeder.json";
...
const StudentinfoSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("studentinfo").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(StudentinfoSeederDialogComponent);
