...
import studentinfoFakerFactory from "./studentinfoFakerFactory";
...
const StudentinfoFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = studentinfoFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("studentinfo").create(elem));
...
export default connect(null, mapDispatch)(StudentinfoFakerDialogComponent);
